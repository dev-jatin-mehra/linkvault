...
export const searchLinks = async (userId, query, limit) => {
...
export const getLinksForCollection = async (userId, collectionId) => {
...
export const updateLink = async (
  ...
  try {
    ...
    if (!accessCheck.rowCount) return null;
...
export const deleteLink = async (userId, id) => {
  const result = await pool.query(
    `DELETE FROM links
     WHERE id = $1
...
export const trackLinkClick = async (userId, id, { userAgent, referrer }) => {
  const client = await pool.connect();
  ...
  try {
    ...
    return { click_count: updateResult.rows[0]?.click_count || 0 };
  } catch (error) {
    await client.query("ROLLBACK");
    throw error;
  } finally {
...
export const trackPublicLinkClick = async (id, { userAgent, referrer }) => {
  const client = await pool.connect();
